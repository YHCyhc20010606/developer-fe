...
<template>
  <div class="detail-sandbox">
      ...
      >
        ...
        >
          ...
          <div class="application-title">
            ...
            <div
              class="add-vm-small rt"
              @click="checkAntiAffinityGroup('check')"
              v-if="appClass==='VM'"
            >
              <img
                src="../../../assets/images/sandbox/anti_affinity_group_icon.png"
                alt=""
              >
            </div>
          </div>
          ...
          >
            <UPF
              :is-upf-disabled-prop="isUpfDisabled"
    ...
    <el-dialog
      class="default_dialog"
      width="1100px"
      v-if="showAntiAffinityGroup"
      :visible.sync="showAntiAffinityGroup"
      :close-on-click-modal="false"
    >
      <AntiAffinityGroup
        @addVmFinish="addVmFinish"
      ...
      >
        <el-button
          id="btn_cancelAddVm"
          class="common-btn"
          @click="checkAntiAffinityGroup('cancel')"
        >
          {{ $t('common.confirm') }}
        </el-button>
      </span>
...
import AntiAffinityGroup from './vm/AntiAffinityGroup.vue'
...
export default {
  ...
  data () {
    return {
      ...
      showAntiAffinityGroup: false,
      showVmDetail: false,
      isAntiAffinityBtn: true,
  ...
  methods: {
    checkAntiAffinityGroup (str) {
      if (str === 'check') {
        this.showAntiAffinityGroup = true
      } else if (str === 'cancel') {
        this.showAntiAffinityGroup = false
      }
    },
    getInternetType () {
    ...
    getApplicationInfo () {
      applicationApi.getAppInfo(this.applicationId).then(res => {
        ...
        this.checkPkgSpecId(res.data.pkgSpecId)
    ...
    checkPkgSpecId (pkgSpecId) {
      applicationApi.getSpecOptions().then(res => {
        if (res.data && res.data.length > 0) {
          let _data = res.data
          _data.forEach(item => {
            if (item.id === pkgSpecId && item.zhName === '移动二期') {
              this.isUpfDisabled = true
            }
          })
        }
      }).catch(err => {
