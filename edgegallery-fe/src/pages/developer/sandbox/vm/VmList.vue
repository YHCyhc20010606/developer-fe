...
<template>
  <div class="details-center-vm">
    ...
    >
        ...
        >
          ...
          >
            ...
            <div
              ...
              :class="(isStartUpVmSuccess && isExportImageFinish) ? 'img-click':'img-onlyRead'"
      ...
      <el-dialog
        :visible.sync="sshDialog"
        :close-on-click-modal="false"
        class="ssh-info-dialog default_dialog"
        append-to-body
        center
      >
        <p class="ssh-info">
          {{ $t('sandboxPromptInfomation.waitSSH') }}
        </p>
      </el-dialog>
    </div>
  </div>
  ...
  methods: {
    ...
    getVmExportStatus (operationId) {
      sandbox.getVmStatus(operationId).then(res => {
        ...
        let _objectId = res.data.objectId
        sandbox.getVmlist(this.applicationId).then(response => {
          let _data = response.data
          _data.forEach(item => {
            if (_objectId === item.id) {
              this.vmImageInformation.imageName = item.imageExportInfo.name
              this.vmImageInformation.status = item.imageExportInfo.status
              this.vmImageInformation.downloadUrl = item.imageExportInfo.downloadUrl
            }
          })
          setTimeout(() => {
    ...
    deleteVm () {
      ...
      sandbox.antiAffinityGroup.getAntiAffinityGroup(this.applicationId, params).then(res => {
        if (res.data.length === 0) {
          ...
          this.$eg_messagebox(this.$t('sandboxPromptInfomation.deleteVmInfo'), 'error')
        }
      })
    },
    checkVmDetail () {
    ...
    exportImage (data) {
      ...
      this.$eg_messagebox(this.$t('sandboxPromptInfomation.exportImagePrompt'), 'warning', this.$t('common.cancel')).then(() => {
        sandbox.exportImage(this.applicationId, data).then(res => {
          this.exportFailed = false
          this.imagePercentages = 0
          this.vmImageInformation.imageName = ''
          this.vmImageInformation.status = ''
          this.vmImageInformation.downloadUrl = ''
          this.isExportImageFinish = false
          this.timerExport = setInterval(() => {
...
.vm-bg .down_div{
  ...
  .ssh-info-dialog .el-dialog{
    width: 500px;
    background: rgba(75,55,154,.9);
  }
  .ssh-info{
    font-size: 18px;
    text-align: center;
