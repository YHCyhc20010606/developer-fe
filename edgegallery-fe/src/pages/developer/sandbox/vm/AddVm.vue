...
<template>
  <div class="addVm">
      ...
      >
        ...
        >
          <span
            class="collapse-title"
            slot="title"
          >{{ $t('sandbox.selectVmflavor') }}<strong class="required">*</strong></span>
            ...
            <div class="defaultFontLight">
              <div class="flex">
                ...
                >
                  ...
                  <el-radio-button :label="$t('sandbox.customSpec')" />
                </el-radio-group>
              </div>
              <p class="flavor-info">
                {{ $t('sandbox.flavorFilterInfo') }}
                <el-button
                  v-if="this.vmInfo.archType===$t('sandbox.customSpec')"
                  class="add-customSpec-btn rt"
                  @click="addSpecForm('add')"
                >
              ...
              >
                <el-table-column width="35px">
                  <template slot-scope="scope">
                    <el-radio
                      ...
                      class="common-radio"
                      @change.native="getCurrentFlavorData(scope.row)"
                    />
                  </template>
                </el-table-column>
                <el-table-column
                ...
                >
                  <template slot-scope="scope">
                    <el-button
                      id="btn_vmFlavor_edit"
                      class="operation-btn-text"
                      @click="addSpecForm(scope.row)"
                    >
                      {{ $t('common.edit') }}
                    </el-button>
                    <el-button
                      id="btn_vmFlavor_delete"
                      class="operation-btn-text"
                      @click="deleteSpecForm(scope.row)"
                    >
                      {{ $t('common.delete') }}
                    </el-button>
                  </template>
        ...
        >
          <span
            class="collapse-title"
            slot="title"
          >{{ $t('sandbox.configureNetworkPorts') }}<strong class="required">*</strong></span>
          ...
          <div class="vm-size">
            ...
            >
              <div class="script-content">
                ...
                <mavon-editor
                  class="common-mavon-editor script-editor"
                  v-model="userData"
                  :toolbars-flag="false"
                  :subfield="false"
    ...
    <el-dialog
      ...
      :visible.sync="customSpecs"
      @close="onClose"
      class="default_dialog"
      append-to-body
    >
      <div
        class="addVm-bg customSpecs"
      >
        <div
...
export default {
  ...
  data () {
    const nameEmpty = (_rule, value, callback) => {
      if (value === '') {
    ...
    const descriptionEmpty = (_rule, value, callback) => {
      if (value === '') {
  ...
  watch: {
    '$i18n.locale': function () {
      ...
      this.vmInfo.archType = this.$t('sandbox.customSpec')
    },
    isAddVmProp (val) {
      this.isAddVm = val
  ...
  methods: {
    ...
    checkNameValue (val) {
      let _reg = /^(?!_)(?!-)(?!\s)(?!.*?_$)(?!.*?-$)(?!.*?\s$)[a-zA-Z0-9_-]{4,15}$/
    ...
    getVmSpecs (systemDiskSize) {
      sandbox.getVmspec().then(res => {
        ...
        this.vmSpecs.forEach(item => {
          if (this.echoFlavorId === item.id) {
            if (item.userId) {
              this.vmInfo.archType = this.$t('sandbox.customSpec')
            } else {
              this.vmInfo.archType = item.architecture
            }
          }
        })
        this.addvmImages.flavorId = this.vmSpecs[0] ? this.vmSpecs[0].id : ''
        this.filterVmRegulation(this.vmInfo.archType)
      }).catch(err => {
        console.log(err)
      })
    },
    ...
    changeImageType (data) {
      ...
      if (data === 'public') {
        this.vmInfo.privateSystemName = ''
        this.vmInfo.privateImageOptions = ''
        this.vmInfo.privateId = ''
      } else {
        this.vmInfo.publicSystemName = ''
        this.vmInfo.publicImageOptions = ''
        this.vmInfo.publicId = ''
      }
    },
    getCurrentFlavorData (flavorData) {
      this.selectedFlavorId = flavorData.id
    },
    filterVmRegulation (val) {
      let _defaultFlavorId = ''
      _defaultFlavorId = this.vmSpecs[0] ? this.vmSpecs[0].id : ''
      if (val === 'X86' || val === 'ARM') {
        this.vmInfo.vmRegulationList = this.vmSpecs.filter(item => (item.architecture === this.vmInfo.archType && !item.userId))
      } else {
        this.vmInfo.vmRegulationList = this.vmSpecs.filter(item => item.userId)
      }
      if (!this.isAddVm) {
        this.addvmImages.flavorId = this.echoFlavorId
      } else {
        this.addvmImages.flavorId = this.selectedFlavorId === '' ? _defaultFlavorId : this.selectedFlavorId
      }
    },
    ...
    changePublicImageSize (val) {
      ...
      sandbox.getVmDetailImage(val).then(res => {
        ...
        this.getVmSpecs(this.systemDiskSize)
        let _params = ''
        let _temp = res.data.osVersion.replaceAll('.', '').replaceAll(' ', '')
        let _version = _temp.indexOf('1804') > -1
        if (res.data.osType === 'Ubuntu' && _version) {
          _params = (res.data.osType.toLowerCase() + '_' + 1804).replaceAll('.', '').replaceAll(' ', '')
        } else if (res.data.osType === 'Ubuntu' && !_version) {
          _params = res.data.osType.toLowerCase().replaceAll('.', '').replaceAll(' ', '')
        } else {
          _params = res.data.osType.toLowerCase().replaceAll(' ', '')
        }
        sandbox.getScriptByImageId(sessionStorage.getItem('pkgSpecId'), _params).then(response => {
          if (response.data) {
            this.userData = '```shell\r\n' + response.data + '\r\n```'
          } else {
    ...
    appendSizeUnit (_row, _column, cellValue) {
      return cellValue + 'GB'
    },
    handleSelectInjectScript (val) {
      ...
      if (!this.isAddVm) {
        this.changePublicImageSize(this.addvmImages.imageId)
      }
    },
    ...
    clickEdit () {
      ...
      let _userData = this.userData.replace('```shell\r\n', '').replace('\r\n```', '')
    ...
    deleteSpecForm (row) {
      this.$eg_messagebox(this.$t('promptInformation.confirmDelete'), 'warning', this.$t('common.cancel')).then(() => {
        sandbox.deleteCustom(row.id, this.userId).then(() => {
          if (row.id === this.echoFlavorId) {
            this.echoFlavorId = ''
          }
          this.getVmSpecs(this.systemDiskSize)
        }).catch(() => {
      ...
      this.$refs['customRule'].validate((valid) => {
        ...
        if (this.isAddFlavor) {
          sandbox.addCustom(this.custom).then(() => {
            ...
            if (error.response.data.message === 'exist same data.') {
              this.$eg_messagebox(this.$t('sandbox.flavorIsExist'), 'error')
            } else {
              this.$eg_messagebox(this.$t('promptInformation.addFailed'), 'error')
            }
          })
        } else {
          sandbox.editCustom(this.custom.id, this.custom).then(() => {
            this.handleCustomData()
          }).catch((error) => {
            if (error.response.data.message === 'exist same data.') {
              this.$eg_messagebox(this.$t('sandbox.flavorIsExist'), 'error')
    ...
    addVmFinish (type) {
      ...
      if (type === 'confirm') {
        if (this.changeResult) {
          this.addvmImages.userData = this.userData.replace('```shell\r\n', '').replace('\r\n```', '')
        } else {
          this.addvmImages.userData = ''
        }
        if (this.isAddVm) {
          this.addvmImages.imageId = this.vmInfo.publicId === '' ? this.vmInfo.privateId : this.vmInfo.publicId
        }
    ...
    checkVmData (_data) {
      if (this.addvmImages.name === '') {
        ...
        this.activeNames = ['4']
        this.$emit('addVmFinish', _data, false)
      } else if (this.addvmImages.flavorId === '') {
        this.$eg_messagebox(this.$t('sandbox.isFlavorEmpty'), 'warning')
        this.activeNames = ['2']
        this.$emit('addVmFinish', _data, false)
    ...
    editVmDetail () {
      ...
      this.bus.$on('editVmDetail', function (data) {
        ...
        _this.echoFlavorId = data.flavorId
      })
    },
    ...
    getVmDetailImage (imageId) {
      ...
      sandbox.getVmDetailImage(imageId).then(res => {
        ...
        if (res.data.visibleType === 'public') {
          this.vmInfo.publicSystemName = res.data.osType
          this.vmInfo.publicId = res.data.name + '[' + res.data.osVersion + ' ' + res.data.osBitType + '(' + res.data.systemDiskSize + 'GB)]'
          this.vmInfo.privateId = ''
          this.vmInfo.privateSystemName = ''
        } else {
          this.vmInfo.privateSystemName = res.data.osType
          this.vmInfo.privateId = res.data.name + '[' + res.data.osVersion + ' ' + res.data.osBitType + '(' + res.data.systemDiskSize + 'GB)]'
          this.vmInfo.publicId = ''
          this.vmInfo.publicSystemName = ''
        }
...
.addVm{
  ...
  .customSpec-table .el-button.operation-btn-text{
