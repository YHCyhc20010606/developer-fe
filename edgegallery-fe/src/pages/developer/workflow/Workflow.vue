...
<script>
import { atpTestApi, applicationApi, sandbox } from '../../../api/developerApi.js'
export default {
  props: {
    currentFlow: {
      type: Number,
  ...
  methods: {
    ...
    jumpToAtp (item) {
      applicationApi.getAppInfo(sessionStorage.getItem('applicationId')).then(res => {
        if (res.data.status === 'PACKAGED') {
          this.$router.push(item.toPath)
        } else {
          atpTestApi.getTestId(sessionStorage.getItem('applicationId')).then(response => {
            if (response.data[0].status === 'running' || response.data[0].status === 'success' || response.data[0].status === 'failed') {
