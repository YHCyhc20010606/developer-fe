        ...
        >
          ...
          >
            ...
            <span
              ...
              v-if="isk8sConfig"
            >
              {{ $t('devSystem.downloadUrl') }}: ~/.kube/config
            </span>
            <a
              v-if="!isk8sConfig"
...
import configOpenStack from '../../../assets/file/config-openstack.yaml'
import configFusionSphere from '../../../assets/file/config-fusionsphere.yaml'
...
export default {
  ...
  data () {
    ...
    const validateMechost = (_rule, value, callback) => {
    ...
    const validateLcmIp = (_rule, value, callback) => {
      let reg = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/
      if (!value) {
    ...
    const validatePort = (_rule, value, callback) => {
    ...
    const validateNetworkParameter = (_rule, value, callback) => {
      if (!value) {
        callback(new Error(`${this.$t('devSystem.pleaseInput')}${this.$t('devSystem.networkConfig')}`))
      } else {
        callback()
      }
    }
    return {
        ...
        portRangeMin: [{ required: true,
          validator: (_r, _v, callback) => {
            validate(['portRangeMin', 'portRangeMax'], callback, `${this.$t('devSystem.pleaseInput')}${this.$t('devSystem.portRange')}`, [{ rule: () => {
              return /^(\d|[1-9]\d{1,3}|[1-5]\d{4}|6[0-5]{2}[0-3][0-5])$/.test(this.form.portRangeMin) && /^([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-5]{2}[0-3][0-5])$/.test(this.form.portRangeMax)
            },
            message: `${this.$t('devSystem.pleaseInput')}${this.$t('devSystem.correct')}${this.$t('devSystem.inPort')}` }])
          } }],
        networkParameter: [
          { required: true, validator: validateNetworkParameter }
        ]
      },
      visible: false,
      allListData: [],
      ...
      configType: 'resource',
      isk8sConfig: true,
      demoConfig: configOpenStack,
      demoOpenStack: configOpenStack,
      demoFusionSphere: configFusionSphere,
      demoName: 'config-openstack',
      isDeleteConfigfile: false
    }
  },
  mounted () {
    this.setDivHeight()
  ...
  watch: {
    ...
    offsetPage (val, _oldVal) {
      this.offsetPage = val
      this.getListData()
    },
    limitSize (val, _oldVal) {
      this.limitSize = val
      this.getListData()
    }
  },
  methods: {
    changeOs (val) {
      this.showOther = val !== 'K8S'
      this.isk8sConfig = val === 'K8S'
      if (val === 'OpenStack') {
        this.demoConfig = this.demoOpenStack
        this.demoName = 'config-openstack'
      } else {
        this.demoConfig = this.demoFusionSphere
        this.demoName = 'config-fusionsphere'
      }
    },
    ...
    confirmData () {
      ...
      if (this.configType === 'resource') {
        this.otherDataResource.forEach(item => {
          nullMum = this.handleNullMun(item, nullMum)
        })
      } else {
    ...
    onSubmit () {
      this.$refs.form.validate((valid, params) => {
        if (valid) {
          ...
          if (this.isDeleteConfigfile) {
            applicationApi.deleteFileApi(this.form.configId).then(() => {
              this.form.configId = ''
            })
          }
          this.saveHostInfoFun(params)
        }
      })
    },
    saveHostInfoFun (params) {
      systemApi.saveHostInfo({ ...this.form, ...params, userId: this.userName }).then(res => {
        ...
        if (error.response.data.message === 'mecHost already exists!') {
          this.$eg_messagebox(this.$t('devSystem.promptMessage.mecHostExist'), 'error')
        } else if (error.response.data.message === 'networkParameter or resource param pattern wrong,each param must split by ; and key value must split by =') {
          this.$eg_messagebox(this.$t('devSystem.promptMessage.hostFormatInfo'), 'error')
        } else if (error.response.data.message === 'Network params config error!') {
          this.$eg_messagebox(this.$t('devSystem.promptMessage.hostNetworkConfigInfo'), 'error')
        } else {
    ...
    handleShowForm (v) {
      ...
      this.showOther = this.form.vimType !== 'K8S'
      this.isk8sConfig = this.form.vimType === 'K8S'
      this.configId_file_list = []
      this.visible = true
      this.$nextTick(() => {
    ...
    getCurrentPageData (_val, pageSize, start) {
