        ...
        >
          <template slot-scope="scope">
            ...
            <el-progress
              ...
              :percentage="scope.row.slimProgress"
            />
          </template>
        </el-table-column>
        <el-table-column
...
export default {
  ...
  methods: {
    ...
    downloadVmImage (row) {
      return imageMgmtService.downloadVmImage(row.id)
    },
    handleSlimProgress () {
      this.imageListData.forEach(item => {
        if (item.operationId && item.imageSlimStatus === 'SLIMMING') {
          imageMgmtService.getOperationInfo(item.operationId).then(res => {
            this.$set(item, 'slimProgress', res.data.progress)
            if (res.data.progress === 100 || res.data.status !== 'ONGOING') {
    ...
    handleDownload (row) {
      this.$eg_messagebox(this.$t('devSystem.imageMgmt.tip.downloadInfo'), 'info', '', this.$t('common.close'), row.downLoadUrl, '600px')
    },
    ...
    getOperationInfo (operationId, id) {
      this.imageListData.forEach(item => {
        if (item.id === id) {
...
.vmlist{
  ...
  .el-button.operation-btn-text:last-child.slim-btn{
