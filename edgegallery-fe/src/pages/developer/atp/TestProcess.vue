        ...
        <div class="toptitle common-div-bg">
          ...
          <div class="header">
            ...
            >
              ...
              <span class="publish-info">{{ $t('atpTestProcess.publishInfo') }}</span>
              <el-button
                class="common-btn publish-btn rt"
...
export default {
  ...
  methods: {
    normalizeScenarioList (scenarioList) {
      if (!Array.isArray(scenarioList)) {
        return []
      }
      return scenarioList.map(item => {
        if (!item) {
          return item
        }
        const isCommunityCommonScenario = item.nameCh === '社区通用场景' ||
          item.nameEn === 'EdgeGallery Community Common Scenario' ||
          item.nameEn === 'EdgeGallery Community Scenario'
        if (isCommunityCommonScenario) {
          return {
            ...item,
            nameCh: '通用场景'
          }
    ...
    publish () {
      ...
      applicationApi.publishApp(this.applicationId, _parameter).then(res => {
        ...
        if (error.response.data.message === 'The capability is exist') {
          this.$eg_messagebox(this.$t('atpTestProcess.capabilityIsExist'), 'warning', '', this.$t('common.confirm'), '', '500px')
        } else {
    ...
    getTaskProcess () {
      atpApi.getTaskApi(this.taskId).then(res => {
        let data = this.normalizeScenarioList(res.data.data.testScenarios || [])
    ...
    .toptitle{
      ...
      .header{
        ...
        .point::before{
          ...
          .publish-btn{
            background: rgba(67,246,173,.7);
            color: #fff !important;
          }
          .el-button.publish-btn.is-disabled{
