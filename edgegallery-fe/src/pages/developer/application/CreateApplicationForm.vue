    ...
    <div>
      ...
      >
        ...
        <el-form-item
          ...
          prop="version"
          :class="{'marginbot-err':isVersionError&&language==='en'}"
        >
          <el-input
...
export default {
  name: 'CreateProComp',
  data () {
    let validateProjectName = (_rule, value, callback) => {
      let reg = /^(?!_)(?!-)(?!\s)(?!.*?_$)(?!.*?-$)(?!.*?\s$)(?!\d+$)[\u4E00-\u9FA5a-zA-Z0-9_-]{4,32}$/
      let regClab = /^(?!_)(?!-)(?!\s)(?!.*?_$)(?!.*?-$)(?!.*?\s$)(?!\d+$)[a-zA-Z0-9_-]{4,32}$/
      if (!value) {
        return callback(new Error(this.$t('incubation.nameTip')))
      } else if (!this.clabRules && !reg.test(value)) {
        callback(new Error(this.$t('incubation.nameRule')))
      } else if (this.clabRules && !regClab.test(value)) {
        callback(new Error(this.$t('incubation.nameRuleClab')))
      } else {
        return callback()
      }
    }
    let validateVersion = (_rule, value, callback) => {
    ...
    let validateProvider = (_rule, value, callback) => {
      let reg = /^[\S]{0,30}$/g
      let regClab = /^[^\u4e00-\u9fa5\s]{0,30}$/g
      if (!value) {
        return callback(new Error(this.$t('incubation.providerTip')))
      } else if (!this.clabRules && !reg.test(value)) {
        callback(new Error(this.$t('incubation.providerRule')))
      } else if (this.clabRules && !regClab.test(value)) {
        callback(new Error(this.$t('incubation.providerRuleClab')))
      } else {
        return callback()
      }
    }
    let validateDescription = (_rule, value, callback) => {
      let reg = /^(?!\s)(?!\d+$)[\S.\s\n\r]{1,1024}$/g
      let regClab = /^(?!\s)(?!\d+$)[^\u4e00-\u9fa5]{1,1024}$/g
      if (!value) {
        return callback(new Error(this.$t('incubation.descTip')))
      } else if (!this.clabRules && !reg.test(value)) {
        callback(new Error(this.$t('incubation.descriptionRule')))
      } else if (this.clabRules && !regClab.test(value)) {
        callback(new Error(this.$t('incubation.descriptionRuleClab')))
      } else {
        return callback()
      }
    }
  ...
  methods: {
    ...
    confirmToModify () {
      ...
      if (this.isDeleteMdfile) {
        applicationApi.deleteFileApi(this.applicationFormData.guideFileId).then(() => {
          this.applicationFormData.guideFileId = ''
        })
      }
      applicationApi.modifyApp(this.appId, this.applicationFormData).then(() => {
        ...
        console.log(err)
        this.$message.error(this.$t('incubation.modifyAppFailed'))
      })
    },
    confirmToCreate () {
      applicationApi.createNewApp(this.applicationFormData).then(res => {
        ...
        this.$message.error(this.$t('incubation.addAppFailed'))
        console.log(err)
      })
    },
    getApplicationInfo (appId) {
      applicationApi.getAppInfo(appId).then(res => {
        this.$store.commit('changeApp', res.data.name)
        Object.keys(this.applicationFormData).forEach((key, _index) => {
          this.applicationFormData[key] = res.data[key]
        })
        this.applicationFormData.status = res.data.status
        this.normalizeApplicationTypeForSelect()
    ...
    /** 编辑旧应用时接口可能仍为历史 type 枚举，映射到下拉仅三项（2026-05-16） */
    normalizeApplicationTypeForSelect () {
      const allowed = ['Image', 'Video', 'Audio']
      const t = this.applicationFormData.type
      ...
      this.applicationFormData.type = legacyToNew[t] || 'Video'
    },
    ...
    initSpecOptions () {
      applicationApi.getSpecOptions().then(res => {
        if (res.data && res.data.length > 0) {
          ...
          this.applicationFormData.pkgSpecId = this.specOptions[0] ? this.specOptions[0].id : ''
  ...
  mounted () {
    let _useScenes = sessionStorage.getItem('useScenes')
    this.clabRules = _useScenes !== 'edgeGallery'
...
<style lang="less">
  ...
  .new-application .el-upload-list{
    width: auto;
  }
