...
<template>
  ...
  >
    ...
    >
      <span :class="developerMode ? 'dev-mode-on' : 'dev-mode-off'">
        {{ developerMode ? '开发者模式' : '普通模式' }}
      </span>
    </div>
  </Navcomp>
</template>
...
import { devNavData } from '../navdata/devNav.js'
import { devNavData as devNavDataCn } from '../navdata/devNavCn.js'
...
export default {
  ...
  mounted () {
    ...
    window.addEventListener('message', (event) => {
      if (event.data.cmd === 'iframePath') {
        this.$router.push(event.data.params.iframePath)
      }
  ...
  beforeDestroy () {
    clearInterval(this.wsMsgSendInterval)
  ...
  methods: {
    ...
    filterMecmMenu () {
      this.jsonData = this.jsonData.filter(item => item.path !== FIRST_LEVEL_MENU_PATH.MECM && item.path !== FIRST_LEVEL_MENU_PATH.SIGNALING)
    },
    filterMenu () {
    ...
    loginFun () {
      loginApi().then(res => {
        ...
        const isGuestByName = userName.toLowerCase() === 'guest'
        const isGuestByRole = authorities.some(item => item.indexOf('GUEST') > -1)
        ...
        this.ifGuest = isGuestByName || isGuestByRole
        if (this.ifGuest) {
          sessionStorage.removeItem('accessToken')
          sessionStorage.removeItem('userAuthorities')
          sessionStorage.removeItem('userNameRole')
          this.enterLoginPage()
          return
        }
        ...
        this.wsMsgSendInterval = setInterval(() => {
          if (this.wsSocketConn && this.wsSocketConn.readyState === WebSocket.OPEN) {
    ...
    filterMenuForClab () {
      applicationApi.getUseScene().then(res => {
        if (res && res.data) {
          sessionStorage.setItem('useScenes', res.data)
    ...
    startHttpSessionInvalidListener (sessId) {
      ...
      this.wsSocketConn.onmessage = function (msg) {
        ...
        clearInterval(_thisObj.wsMsgSendInterval)
    ...
    enterLoginPage () {
      ...
      window.location.replace(this.loginPage + '&return_to=' + window.location.origin + PROXY_PREFIX_CURRENTSERVER + '&lang=' + _lang)
    },
    os () {
    ...
    changeLange () {
      // Language switch is temporarily disabled.

    },
    _insertDevMenu (language) {
      const lang = language || localStorage.getItem('language') || 'cn'
      const devItem = lang === 'en'
        ? { id: '1', name: 'Developer', path: FIRST_LEVEL_MENU_PATH.DEVELOPER, display: true, children: devNavData }
        : { id: '1', name: '集成开发', path: FIRST_LEVEL_MENU_PATH.DEVELOPER, display: true, children: devNavDataCn }
      const newJsonData = [...this.jsonData]
      newJsonData.splice(1, 0, devItem)
...
.navgation.clearfix .user_right .language {
...
.navgation .logo {
  ...
  width: 130px;
  background: url('/mec.png') no-repeat center / contain;
}
</style>
